...
function products_read_all($db) {
    $stmt = $db->query('SELECT id, slug, title, category, description, image_url, price, price_note, price_on_request, sort_order, created_at, updated_at FROM products ORDER BY sort_order ASC, id ASC');
...
function products_read_one($db, $id) {
    $stmt = $db->prepare('SELECT id, slug, title, category, description, image_url, price, price_note, price_on_request, sort_order, created_at, updated_at FROM products WHERE id = ? LIMIT 1');
    $stmt->execute([$id]);
...
try {
    switch ($method) {
        ...
        case 'POST':
            ...
            $price_on_request = !empty($data->price_on_request) ? 1 : 0;
            ...
            $stmt->execute([$slug, $title, $category, $description, $image_url, $price, $price_note, $price_on_request, $sort_order]);
        ...
        case 'PUT':
            ...
            $stmt = $db->prepare('UPDATE products SET slug = ?, title = ?, category = ?, description = ?, image_url = ?, price = ?, price_note = ?, price_on_request = ?, sort_order = ?, updated_at = NOW() WHERE id = ?');
            $stmt->execute([$slug, $title, $category, $description, $image_url, $price, $price_note, $price_on_request, $sort_order, $id]);
